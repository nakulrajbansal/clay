...
from the status token. onCardMove makes cards DRAGGABLE between columns
(bidirectional, reversible) — carry each card's id and update its status
to the target column; the table it writes appears in declared_writes.
