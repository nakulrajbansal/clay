...
/**
 * Release C acquisition and worker payload ceilings. Values are literals so a
 * boundary cannot quietly negotiate weaker limits or reintroduce truncation.
 * C-FR-004, C-FR-024, C-NFR-003, C-NFR-006.
 */
export const ImportAcquisitionLimitsSchema = z.object({
...
export const IMPORT_ACQUISITION_LIMITS: ImportAcquisitionLimits = Object.freeze({
  maxDataRows: 5_000,
  maxMappedColumns: 20,
...
export const ImportSessionIdSchema = z.string().regex(/^import_[a-z2-7]{26}$/);
export const ImportSheetIdSchema = z.string().regex(/^[a-z][a-z0-9_-]{0,63}$/);
export const ImportSourceKindSchema = z.enum([
  "csv", "xlsx", "paste", "tsv_compat", "json_compat",
]);

export const ImportSourceRangeSchema = z.object({
  rows: z.number().int().nonnegative().max(5_001),
  columns: z.number().int().nonnegative().max(20),
}).strict();

export const ImportSheetDescriptorSchema = z.object({
...
export const ImportSourceDescriptorSchema = z.object({
  version: z.literal(1),
...
export const ImportSheetChoiceSchema = z.object({
  sheetId: ImportSheetIdSchema,
}).strict();

export const ImportRangeChoiceSchema = z.object({
  sheetId: ImportSheetIdSchema,
...
export const ImportHeaderChoiceSchema = z.discriminatedUnion("mode", [
  z.object({
...
export type ImportHeaderChoice = z.infer<typeof ImportHeaderChoiceSchema>;

export const HeaderCandidateReasonSchema = z.enum([
  "no_non_blank_row",
  "first_non_blank_row",
  "all_labels_present",
...
export const HeaderCandidateSchema = z.object({
...
export const ImportDateRuleSchema = z.discriminatedUnion("kind", [
...
export type ImportDateRule = z.infer<typeof ImportDateRuleSchema>;

export const ImportNumberRuleSchema = z.object({
  grammar: z.enum([
...
export type ImportNumberRule = z.infer<typeof ImportNumberRuleSchema>;

export const ImportSkipCodeSchema = z.enum([
  "above_header",
  "blank_row",
  "user_skipped",
  "duplicate_combined",
...
const SourceRowOrdinalSchema = z.number().int().positive().max(5_001);
const ImportIssueIdSchema = z.string().regex(/^issue_[a-zA-Z0-9_-]{1,64}$/);

export const SourceRowDispositionSchema = z.discriminatedUnion("kind", [
...
export const ImportSourceDispositionLedgerSchema = z.array(SourceRowDispositionSchema)
...
const BoundedImportCountSchema = z.number().int().nonnegative().max(5_000);
export const ImportSkipReasonCountsSchema = z.object({
...
export const SourceDispositionTotalsSchema = z.object({
...
export const MutationTotalsSchema = z.object({
...
const ImportMutationIdSchema = z.string().regex(/^mutation_[a-z2-7]{26}$/);
const ImportTableIdSchema = z.string().regex(/^tbl_[a-z2-7]{26}$/);
const ImportRowIdSchema = z.string().regex(
  /^[0-9a-f]{8}-[0-9a-f]{4}-[1-8][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i,
);
const OriginSourceRowsSchema = z.array(SourceRowOrdinalSchema).min(1).max(5_000)
...
export const PreparedImportMutationSchema = z.discriminatedUnion("role", [
  z.object({
...
export const SourceRowMutationMapSchema = z.object({
  sourceRow: SourceRowOrdinalSchema,
...
export const ImportPreparedLedgerSchema = z.object({
...
export const ImportParserErrorCodeSchema = z.enum([
...
export const ImportParserStageSchema = z.enum([
  "acquire", "decode", "parse", "chunk", "session", "protocol",
]);
const ImportParserRpcIdSchema = z.number().int().positive().safe();
...
export const OpenImportSourceRequestSchema = z.object({
  version: z.literal(1),
...
export const ReadImportChunkRequestSchema = z.object({
  version: z.literal(1),
  id: ImportParserRpcIdSchema,
...
export const CloseImportSourceRequestSchema = z.object({
  version: z.literal(1),
  id: ImportParserRpcIdSchema,
...
export const ImportParserRequestSchema = z.discriminatedUnion("op", [
  OpenImportSourceRequestSchema,
  ReadImportChunkRequestSchema,
  CloseImportSourceRequestSchema,
]);

const ImportRawCellSchema = z.string().max(16 * 1024);
export const ImportParserChunkSchema = z.object({
  sessionId: ImportSessionIdSchema,
  cursor: z.number().int().nonnegative().max(5_000),
  startRow: z.number().int().positive().max(5_001),
...
export const CloseImportSourceResultSchema = z.object({ disposed: z.literal(true) }).strict();

export const ImportParserSafeErrorSchema = z.object({
...
export const ImportParserResponseSchema = z.union([
  z.object({
    version: z.literal(1),
    id: ImportParserRpcIdSchema,
    ok: z.literal(true),
