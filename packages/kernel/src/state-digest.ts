...
class Sha256Sync {
  ...
  update(input: Uint8Array): this {
    if (this.finalized) throw new Error("SHA-256 is already finalized");
  ...
  private compress(input: Uint8Array, offset: number): void {
    ...
    for (let index = 16; index < 64; index++) {
      ...
      this.words[index] = (this.words[index - 16]! + sigma0
        + this.words[index - 7]! + sigma1) >>> 0;
    }
    ...
    for (let index = 0; index < 64; index++) {
      ...
      const temp1 = (h + sum1 + choose + ROUND[index]! + this.words[index]!) >>> 0;
