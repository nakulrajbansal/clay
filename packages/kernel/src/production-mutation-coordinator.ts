...
import {
  captureFirstRunImportUndo,
  captureTableImport,
  executeCapturedFirstRunImportUndo,
  executeCapturedTableImport,
  type CapturedFirstRunImportUndo,
...
type CapturedProductionMutation = CapturedCoreMutation | Readonly<{
  ...
  | { route: "firstRun.undoImport"; payload: CapturedFirstRunImportUndo }
...
function captureMutation(input: unknown): CapturedProductionMutation {
  try {
    ...
    switch (route) {
      ...
      case "firstRun.undoImport": return done(captureFirstRunImportUndo(payload));
...
function usesSampleProvenance(route: string): boolean {
  return isSampleProvenanceRoute(route) || route === "samples.remove";
}

function executeCapturedMutationWithoutSampleHandoff(
  store: ClayStore,
  ...
  switch (request.route) {
    ...
    case "firstRun.undoImport":
      return capturedExecution(captureJsonValue(
        executeCapturedFirstRunImportUndo(store, request.payload), new WeakSet(),
      ));
    case "samples.remove":
      return capturedExecution(captureJsonValue(
...
function executeCapturedMutation(
  ...
  const personalizing = isSampleProvenanceRoute(request.route)
    && !isSampleProducingRoute(request.route);
  const before = personalizing ? captureActiveSampleSnapshots(store) : Object.freeze([]);
  const execution = executeCapturedMutationWithoutSampleHandoff(
    store, request, executionInstant, operationId,
  );
  if (!personalizing) return execution;
  const touched = request.route === "attachment.add" || request.route === "attachment.remove"
    ? [Object.freeze({ table: request.payload.table, rowId: request.payload.rowId })]
    : [];
  const promoted = promotePersonalizedSampleRows(store, before, touched);
  return capturedExecution(execution.result, promoted);
}
...
export class ProductionMutationCoordinator {
  ...
  async #executeCaptured(request: CapturedProductionMutation): Promise<ProductionMutationResult> {
    ...
    try {
      ...
      const preparedExecution = executeCapturedMutation(
        shadow, request, executionInstant, operationId,
      );
  ...
  #executeNoOp(
    ...
    const encoded = encodeProductionResponse(
      request.route, result, isSampleProvenanceRoute(request.route) ? [] : undefined,
    );
    this.#writeAuthority.run(() => {
    ...
    try {
      ...
      const committedState = this.#writeAuthority.run(() => {
        ...
        const execution = executeCapturedMutation(
          this.#store, request, executionInstant, operationId,
        );
  ...
  #recordAbandonment(
    ...
    try {
      ...
      const encoded = encodeProductionResponse(
        route, failure, isSampleProvenanceRoute(route) ? [] : undefined,
      );
      const at = trustedInstant(this.#clock);
      this.#writeAuthority.run(() => {
        const { catalog, target } = this.#authorityState(
