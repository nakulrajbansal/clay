...
export async function deleteBrowserNamespaceStorage(
  ...
  for (const file of [namespace.userFile, namespace.systemFile]) {
  ...
  if (after.has(namespace.userFile) || after.has(namespace.systemFile))
    throw new ClayError("E_CATALOG_UNAVAILABLE", "durable namespace cleanup was incomplete");
}
