...
export type SamplePromotionRoute = typeof SAMPLE_PROMOTION_ROUTES[number];
export type SampleProvenanceRoute = SampleProducerRoute | SamplePromotionRoute;

export function isSamplePromotionRoute(route: string): route is SamplePromotionRoute {
  return (SAMPLE_PROMOTION_ROUTES as readonly string[]).includes(route);
}
...
export function sampleProvenanceRouteForOperationId(
  authorityIncarnationId: string,
  requestId: string,
  operationId: string,
): SampleProvenanceRoute | null {
  const producer = sampleProducerRouteForOperationId(
    authorityIncarnationId, requestId, operationId,
  );
  if (producer !== null) return producer;
  let matched: SamplePromotionRoute | null = null;
  for (const route of SAMPLE_PROMOTION_ROUTES) {
    if (productionOperationIdV2(authorityIncarnationId, requestId, route) !== operationId)
      continue;
    if (matched !== null)
      throw invalid("production operation identity matches multiple provenance routes");
    matched = route;
  }
  return matched;
}
