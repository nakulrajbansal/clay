...
const STORE_PROMOTE_PROVENANCE: ClayStore["promoteSampleRowProvenance"] =
  ClayStore.prototype.promoteSampleRowProvenance;
...
export type ActiveSampleSnapshot = Readonly<{
  tableId: string;
  tableName: string;
  rowId: string;
...
function userValueFingerprint(row: Readonly<Record<string, unknown>>): string {
...
/** Capture only user-visible values for active authenticated samples. */
export function captureActiveSampleSnapshots(store: ClayStore): readonly ActiveSampleSnapshot[] {
  const snapshots: ActiveSampleSnapshot[] = [];
  for (const entry of verifiedProvenance(store)) {
    if (!entry.tableActive || entry.rowState !== "active") continue;
    ...
    if (!row) throw invalid("sample provenance references a missing active row");
    snapshots.push(Object.freeze({
      tableId: entry.tableId,
      tableName: entry.tableName,
      rowId: entry.rowId,
      valueFingerprint: userValueFingerprint(row),
    }));
  }
...
/**
 * Promote every sample whose visible values changed, plus exact coordinates
 * touched by attachment routes. Run in the same disposable/live authority
 * transaction as the mutation so a provenance failure rolls the write back.
 */
export function promotePersonalizedSampleRows(
  store: ClayStore,
  before: readonly ActiveSampleSnapshot[],
  touched: readonly Readonly<{ table: string; rowId: string }>[] = [],
): readonly SampleProvenanceCoordinate[] {
  const byCoordinate = new Map<string, ActiveSampleSnapshot>(before.map(snapshot =>
    [`${snapshot.tableName}\u0000${snapshot.rowId}`, snapshot] as const));
  const promote = new Set(touched.map(item => `${item.table}\u0000${item.rowId}`));
  for (const snapshot of before) {
    const row = STORE_QUERY.call(store, {
      from: snapshot.tableName,
      where: [{ field: "id", op: "eq", value: snapshot.rowId }],
      includeDeleted: true,
      limit: 1,
    })[0];
    if (row && row.deleted_at == null
        && userValueFingerprint(row) !== snapshot.valueFingerprint)
      promote.add(`${snapshot.tableName}\u0000${snapshot.rowId}`);
  }
  const promoted: SampleProvenanceCoordinate[] = [];
  for (const coordinate of [...promote].sort()) {
    const split = coordinate.indexOf("\u0000");
    const snapshot = byCoordinate.get(coordinate);
    if (STORE_PROMOTE_PROVENANCE.call(
      store, coordinate.slice(0, split), coordinate.slice(split + 1),
    )) {
      if (!snapshot) throw invalid("promoted sample coordinate was not captured");
      promoted.push(Object.freeze({ tableId: snapshot.tableId, rowId: snapshot.rowId }));
    }
  }
  return Object.freeze(promoted.sort((left, right) =>
    `${left.tableId}\u0000${left.rowId}`.localeCompare(`${right.tableId}\u0000${right.rowId}`)));
}
