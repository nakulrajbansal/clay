...
describe("authenticated sample provenance proof", () => {
  ...
  it("binds the catalog reservation to the asserted authority incarnation", () => {
    expect(() => assertAuthenticatedSampleProvenance({
      ...proof,
      catalogReservations: [{
        ...catalogReservation,
        authorityIncarnationId: id("auth", "z"),
      }],
    })).toThrow(/authority incarnation|reservation evidence diverges/i);
  });
