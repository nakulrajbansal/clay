...
describe("production starter seed authority", () => {
  ...
  it("publishes starter progress and its sample receipt in the same certified revision", async () => {
    const authority = await freshAuthority("e");
    try {
      const request = {
        ...
        payload: {
          ...trackerBundle(),
          activation: { operationId: "starter-operation-0001", appId: "default" },
        },
      };
      const committed = await authority.executeMutation(request);
      expect(committed).toMatchObject({ changed: true, result: null,
        evidence: { protectionRevision: "1" } });
      const publication = authority.readSetting<Record<string, unknown>>(
        "first_run_publication_v1",
      )!;
      expect(publication).toMatchObject({
        version: 1,
        operationId: "starter-operation-0001",
        appId: "default",
        kind: "starter",
        shellId: "tracker",
        undone: false,
        sampleCreation: { route: "starter.seed", created: expect.any(Array) },
      });
      expect((publication.sampleCreation as { created: unknown[] }).created).toHaveLength(4);
