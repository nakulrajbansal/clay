...
describe("production sample-row authority", () => {
  ...
  it("promotes user-edited samples inside the certified update and batch transactions", async () => {
    const authority = await freshAuthority("h");
    try {
      await authority.executeMutation({
        requestId: opaque("req", "h"), route: "starter.seed", payload: sampleSeed(),
      });
      ...
      const noOp = await authority.executeMutation({
        ...
        route: "store.update",
        payload: { table: "items", id: first, patch: { title: "Alpha" } },
      });
      expect(noOp.changed).toBe(false);
      expect(authority.sampleRowCount()).toBe(2);

      const edited = await authority.executeMutation({
        requestId: opaque("req", "j"),
        route: "store.update",
        payload: { table: "items", id: first, patch: { title: "Mine" } },
      });
      expect(edited.changed).toBe(true);
      expect(authority.sampleRowCount()).toBe(1);

      const batched = await authority.executeMutation({
        requestId: opaque("req", "k"),
        route: "batch.apply",
        payload: {
          source: "user",
          summary: "Personalize sample",
          mutations: [
            { kind: "update", table: "items", id: second, patch: { title: "Final" } },
          ],
        },
      });
      expect(batched.changed).toBe(true);
      expect(authority.sampleRowCount()).toBe(0);

      const removed = await authority.executeMutation({
        requestId: opaque("req", "l"), route: "samples.remove", payload: {},
      });
      expect(removed.result).toMatchObject({ affected: 0 });
      expect(authority.query({ from: "items" }).map(row => row.title)).toEqual(["Mine", "Final"]);
    } finally {
      authority.close();
    }
  });
