...
describe("ciphertext-only intake relay", () => {
  it("refuses unauthenticated form allocation when account auth is configured", async () => {
    ...
    const linkToken = await auth.sessions.issueLink(user);
    if (!linkToken) throw new Error("test link was not issued");
    const session = await auth.sessions.redeem(linkToken);
    expect(session).not.toBeNull();
    if (!session) throw new Error("test session was not issued");
  ...
  it("transactionally limits active form allocation per publisher and source", async () => {
    ...
    const sessionFor = async (email: string): Promise<string> => {
      ...
      const link = await auth.sessions.issueLink(user);
      if (!link) throw new Error("test link was not issued");
      const session = await auth.sessions.redeem(link);
      if (!session) throw new Error("test session was not issued");
      return session;
    };
