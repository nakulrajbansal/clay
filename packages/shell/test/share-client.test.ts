...
describe("strict browser share relay client", () => {
  it("rejects insecure remote origins and never promotes absence/expiry to a terminal acknowledgement", async () => {
    ...
    await expect(client.revoke(shareId, token)).rejects.toMatchObject({ status: 410 });
  });
  it("requires an exact terminal receipt, including the immutable request digest", async () => {
    const client = new BrowserShareRelayClient("https://relay.example", null,
      async () => response({ schema: 1, shareId, expiresAt: request.expiresAt, requestSha256: "0".repeat(64), terminal: true }));
    await expect(client.terminalize(request, token)).rejects.toThrow(/terminal|identity|capability/i);
  });
