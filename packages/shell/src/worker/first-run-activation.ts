...
import type { ImportReviewSummary } from "../app/ImportReview";
import {
...
import {
  ...
  type StarterSeedBundle,
  type StarterShellId,
} from "../shells/seed";
import {
  parseSampleCreatedResult, readSampleProvenance, recordSampleRows,
...
export type StarterSeedStage =
  | "structure-staged" | "records-staged" | "provenance-staged";
export type StarterSeedResult = Readonly<{
  route: "starter.seed";
  created: readonly { tableId: string; rowId: string; operationId: string }[];
}>;
...
function materializeStarterRow(
  table: StarterSeedBundle["tables"][number],
...
function seedStarterShell(
  store: ClayStore,
  shellId: StarterShellId,
  fault?: (stage: StarterSeedStage) => void,
): StarterSeedResult {
  const bundle: StarterSeedBundle = createStarterSeedBundle(shellId);
  const seedInstant = new Date().toISOString();
  const operations: MigrationPlanT["operations"] = bundle.tables.map(table => ({
    op: "create_table",
    table: table.name,
    columns: table.columns,
  }));
  store.commit({
    intent: `Start from ${bundle.shellName}`,
    summary: `${bundle.shellName} starter`,
    migration: operations.length === 0
      ? null
      : { operations, inverse: deriveInverse(operations, store.registrySnapshot()) },
    panels: bundle.panels,
  });
  fault?.("structure-staged");
  const sampleIds: Record<string, string[]> = {};
  for (const table of bundle.tables) {
    sampleIds[table.name] = [];
    for (const row of table.sampleRows) {
      const inserted = store.insert(
        table.name, materializeStarterRow(table, row, seedInstant),
      );
      sampleIds[table.name]!.push(String(inserted.id));
    }
  }
  fault?.("records-staged");
  const created = Object.values(sampleIds).some(ids => ids.length > 0)
    ? recordSampleRows(store, sampleIds) : Object.freeze([]);
  fault?.("provenance-staged");
  return Object.freeze({
    route: "starter.seed" as const,
    created,
  });
}

function recordProvenanceSummary(store: ClayStore): {
  sampleCount: number; sampleTables: string[]; realRecordCount: number; provenanceValid: boolean;
} {
  const samples = readSampleProvenance(store);
  const sampleKeys = new Set(samples.map(item => JSON.stringify([item.tableId, item.rowId])));
  let realRecordCount = 0;
  for (const table of store.registrySnapshot().values()) {
    const tableId = table.semantic?.tableId ?? table.name;
    for (const row of store.query({ from: table.name, includeDeleted: false }))
      if (!sampleKeys.has(JSON.stringify([tableId, String(row.id)]))) realRecordCount++;
  }
  return {
    sampleCount: samples.length,
    sampleTables: [...new Set(samples.map(item => item.tableId))].sort(),
    realRecordCount,
...
async function publishStage(
  ...
  const published = await canonical.replaceFromArchive(archive);
  const duringReceipt = readFirstRunPublication(published.store, receipt.appId);
  if (JSON.stringify(duringReceipt) !== JSON.stringify(receipt))
