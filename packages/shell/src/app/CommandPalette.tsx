...
export function CommandPalette(props: {
  ...
  const create = async (event: FormEvent): Promise<void> => {
    ...
    try {
      ...
      if (!tableId) throw new Error("Quick capture requires a stable record type identity");
      const receipt = await props.worker.quickCapture(creating.name, row, String(tableId));
      ...
      props.onInfo(`Created in ${humanize(creating.name)} with a durable undo receipt.`, {
        label: "Undo",
        run: () => {
          void props.worker.undoQuickCapture(receipt.id).then(() => {
            props.onWrite(creating.name);
