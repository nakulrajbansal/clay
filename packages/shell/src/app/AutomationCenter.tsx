...
import type {
  ...
  AutomationRecipeCardV1, AutomationRecipeDraftRequestV1, AutomationRecipeOptionV1,
...
function firstOfKind<T extends AutomationRecipeOptionV1["kind"]>(
  recipe: AutomationRecipeCardV1,
  kind: T,
): Extract<AutomationRecipeOptionV1, { kind: T }> {
  const option = recipe.options.find(candidate => candidate.kind === kind);
  if (!option) throw new Error("This recipe is no longer eligible for the current schema.");
  return option as Extract<AutomationRecipeOptionV1, { kind: T }>;
}
...
export function AutomationCenter(props: {
  ...
  const customBuilder = <section className="automation-builder">
    <div className="automation-builder-title"><button className="link"
      ...
      <div><strong>{repairing ? "Repair this older rule in place" : "Build a custom local rule"}</strong>
        <span>{repairing
    ...
    <footer className="automation-builder-actions"><button onClick={() => {
      ...
      {!simulation ? <button className="primary" disabled={busy || !draft.name.trim() || !trace}
      ...
      <div className="automation-body">
        {tab === "rules" ? building ? customBuilder : recipeSetup ? (
          ...
          <section className="automation-recipes" aria-labelledby="automation-recipes-title">
            ...
            <div className="automation-recipe-grid">
              {recipes.map(recipe => <article className="automation-recipe" key={recipe.id}>
                ...
                <button className="primary" onClick={() => setRecipeSetup(recipe)}>Set up recipe</button>
              </article>)}
            </div>
          </section>
          ...
          {pendingEnable ? <div className="automation-enable-preview" aria-live="polite">
            ...
            <button className="primary" disabled={busy} onClick={() => void confirmPendingEnable()}>Enable rule</button>
          </div> : null}
          {pendingRun ? <div className="automation-enable-preview" aria-live="polite">
            ...
            <button className="primary" disabled={busy} onClick={() => void confirmPendingRun()}>Confirm run</button>
          </div> : null}
            ...
            : rules.map(rule => {
              ...
              return <article className={`automation-rule${rule.needsRepair ? " needs-repair" : ""}`} key={rule.id}>
                <button className={`automation-toggle${rule.enabled ? " on" : ""}`}
                  ...
                  disabled={busy || rule.needsRepair}
                ...
                {rule.needsRepair ? <button onClick={() => {
                  ...
                  : <button onClick={() => void previewRun(rule)} disabled={busy}>Preview run</button>}
              ...
              return <article key={run.id}><span className={`run-status ${run.status}`} />
                ...
                {run.status === "failed" ? <div className="automation-run-detail"><code>{run.errorCode}</code>
                  ...
                  : run.undone ? <span className="run-undone">Undone</span>
                    : runRuntime?.undo.available ? <button disabled={busy}
