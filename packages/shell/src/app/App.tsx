...
import {
  ...
  type BatchReceipt, type ClayNotification,
...
} from "./automatic-backup-trigger.browser";
...
function startBrowserDownload(fileName: string, bytes: ArrayBuffer, contentType: string): void {
...
export function App(): React.JSX.Element {
  ...
  const exportRecoveryKit = async (): Promise<void> => {
    ...
    try {
      ...
      new Uint8Array(enrollment.bytes).fill(0);
    }
  ...
  const chooseBackupFolder = async (): Promise<void> => {
    ...
    const authorization = await adapter.authorizeFromUserGesture(currentId);
  ...
  const reauthorizeBackupFolder = async (): Promise<void> => {
    const adapter = backupAdapterRef.current;
    if (!adapter || !backupTargetHint) return;
    const authorization = await adapter.reauthorizeFromUserGesture(backupTargetHint.target);
    if (authorization.status === "unavailable") {
      setBackupTarget(null);
      recordBackupFailure(closedBackupFailureCode(authorization.reasonCode));
      return;
    ...
    const handle = await adapter.reacquire(authorization.target);
    const next = Object.freeze({ target: authorization.target, folderName: handle.name });
    try { saveProductionBackupTarget(localStorage, next); }
    catch { /* the live structured-cloned handle remains authoritative */ }
    setBackupTargetHint(next);
    setBackupTarget(next);
    const trust = await client().backupTrustStatus();
    setBackupTrustStatus(trust);
    if (trust.status === "ready") await runAutomaticBackup(next, "retry");
  };
  ...
  const restoreRecoveryRecord = async (
    ...
    try {
      await client().restoreRow(candidate.table, candidate.id);
  ...
  const undoRecoveryBatch = async (batch: BatchReceipt): Promise<boolean> => {
    ...
    try {
      await client().undoBatch(batch.id);
      for (const table of registryTables) liveBridge?.notifyWrite(table.name);
      await Promise.all([refreshPanels(), refreshRecoveryState()]);
      return true;
    } catch (error) {
  ...
  useEffect(() => {
    ...
    const inspect = async (): Promise<void> => {
      ...
      } catch { /* the Recovery Center reports the durable failure on an attempted run */ }
    };
    void inspect();
