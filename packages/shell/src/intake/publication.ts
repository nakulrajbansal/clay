...
export class IntakePublication {
  ...
  begin(proposal: Proposal): void {
    ...
    this.persist({ schema: 1, formId: mintIntakeFormId(), source, configuration: this.configuration, proposal: Proposal.parse(proposal),
  ...
  async resume(): Promise<{ localForm: LocalIntakeFormV2; link: string }> {
    ...
    if (!job.save) job = this.persist({ ...job, save: this.intent(job.source, "intake.saveForm", { form: draft }) });
    ...
    if (!job.publish) job = this.persist({ ...job, publish: this.intent(saved.target, "intake.markPublished", { formId: job.formId, publishedAt: new Date().toISOString() }) });
    ...
    if (!job.relayConfirmed) {
      ...
      job = this.persist({ ...job, relayInvoked: true }); // Do not let an ambiguous POST become a fresh form ID.
      ...
      try { response = await this.fetchImpl(`${this.configuration.relayBaseUrl}intake/forms`, { method: "POST", credentials: "include", redirect: "error", headers: { "content-type": "application/json" },
        body: JSON.stringify({ schema: 1, formId: job.formId, ownerToken: privateForm.ownerToken, submitToken: privateForm.publicForm.delivery.submitToken,
          expiresAt: draft.publicForm.delivery.expiresAt, maxCiphertextBytes: 12 * 1024 * 1024 }) }); }
      ...
      job = this.persist({ ...job, relayConfirmed: true });
    }
    ...
    this.persist({ ...job, complete: localForm });
  ...
  finish(): void {
    ...
    this.session.cache.removeItem(this.key); if (this.pending()) throw new Error("Intake publication cleanup needs retry");
  }
}
