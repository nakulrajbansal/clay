...
const entries = [
  ...
  ["B", "Folder selection and automatic prepare/readback/publish/retention", ["backupSelection", "backupRecords", "validateBackupStage", "publishBackup"], ui("production-backup.browser.ts"), test("automatic-backup-recovery.test.ts"), "Partial: trust/publication/retention acknowledgement and safe unpublished retirement connected; per-file durable retention accounting remains"],
  ...
  ["C", "Text-to-relation shadow Preview/Keep/replay/Undo", ["previewRelationConversion", "convertTextToRelation", "mutationOutcome", "undoRelationConversion"], ui("RelationConversionDialog.tsx"), test("relation-conversion-recovery.test.tsx"), "Connected: retained immutable Keep/Undo across teardown/reload; read-only historical acknowledgement; exact unchanged-target Undo; stale/failed intent reset UX remains"],
  ...
  ["D", "Quick Capture/date resolution/Undo", ["dailyHomeResolveDate", "dailyHomeQuickCapture", "dailyHomeUndoCapture", "mutationOutcome"], ui("CommandPalette.tsx"), test("quick-capture.test.tsx"), "Connected: immutable app-bound capture/date/request retained in session storage through reload; authority receipt-first acknowledgement and bounded Undo"],
